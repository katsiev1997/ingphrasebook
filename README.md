...
Приложение можно установить как Progressive Web App (PWA):

- Web App Manifest с иконками для домашнего экрана
- Service Worker (регистрация; офлайн-кэш пока не подключён)
- Подсказка «На экран Домой» для iOS
- Адаптивный дизайн для мобильных устройств

Для проверки установки локально используйте HTTPS (`next dev --experimental-https`).
