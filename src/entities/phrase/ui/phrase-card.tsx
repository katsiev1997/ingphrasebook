...
export function PhraseCard({
  ...
  const isFavorite =
    favoritePhrases?.findIndex((item) => item.id === id) !== -1;
  const { mutate: toggleFavorite, isPending: isFavoritePending } =
    useToggleFavorite(isFavorite);
  const { mutate: updatePhrase, isPending: isUpdatePending } =
    useUpdatePhrase();
  const { mutate: deletePhrase, isPending: isDeletePending } =
    useDeletePhrase();
      ...
      <AnimatePresence initial={false}>
        {!isEditing && (
          <motion.div
            ...
            transition={{ duration: 0.18 }}
            className="flex items-center gap-4 overflow-hidden text-sm text-muted-foreground"
          >
